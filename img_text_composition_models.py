...
#         self.name = 'model_name'
    ...
    def compute_loss(self,
        ...
        if self.name == 'composeAE':
            CONJUGATE = Variable(torch.cuda.FloatTensor(64, 1).fill_(-1.0), requires_grad=False)
...
class ComplexProjectionModule(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        #print('Debugging : ')
        #print(CONJUGATE.shape, ' ', delta.shape)
...
class ComposeAE(ImgEncoderTextEncoderBase):
    ...
    def compose_img_text_features(self, img_features, text_features, CONJUGATE = Variable(torch.cuda.FloatTensor(64, 1).fill_(1.0), requires_grad=False)):
